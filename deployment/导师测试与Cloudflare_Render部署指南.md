# 淘金币游戏 Agent 导师测试与部署指南

## 1. 部署目标与边界

本部署用于项目展示、导师评审和规则验收，不接入真实淘金币业务流。
...
- 状态：最近 200 条测试审计仅保存在 Render 进程内存中，服务重启后自动清空。
...
导师还可以修改关卡、失败次数、设备、加载时间、优惠券、加购/下单状态后再次运行。页面会展示：
...
```text
https://<project>.pages.dev,https://demo.example.com
```
...
```text
项目测试地址：https://<project>.pages.dev
...
Render Free Web Service 空闲 15 分钟后会休眠，首次唤醒可能需要约一分钟。若使用免费实例，应在测试前 2 分钟打开 `/health` 预热，并提前告知导师首次连接可能较慢。

若导师需要在固定时间稳定测试，建议测试期间临时升级为不会休眠的付费实例，测试结束后再降级。

### 数据持久化

当前无需数据库。Render 默认文件系统是临时的，本项目也不依赖本地写盘。若未来需要保存导师反馈，应新增独立表单和 Postgres，不要把测试记录写入服务本地文件。
